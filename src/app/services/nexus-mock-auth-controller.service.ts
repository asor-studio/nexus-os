...
import { LoginConnectDataSet } from "../config/nexus-state.config";
...
export class NexusMockAuthControllerService {
    ...
    public authCheck(req: HttpRequest<any>, next: HttpHandler): HttpResponseBase {
        return new HttpResponse({
            ...
            body: {
                authorized: this.checkRequirementsRoutePermission() ? AsorGlobalEnum.AuthStatus.Ok : AsorGlobalEnum.AuthStatus.NotAllow,
            } as IAuth,
        });
    }

    private checkRequirementsRoutePermission(): boolean {
        const user = this._stateService.readDataSet(LoginConnectDataSet.selectors['user']);
